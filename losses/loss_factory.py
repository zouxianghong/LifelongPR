...
def make_mi_loss():
    loss_fn = InfoNCE(temperature=configs.train.loss.mi.temperature)
    return loss_fn
...
def make_inc_loss():
    ...
    elif inc_loss_name == 'DistributionAware':
        loss_fn = DistributionAware()
    elif inc_loss_name == 'InfoNCE':
        loss_fn = InfoNCE(temperature=configs.train.loss.mi.temperature)
    else:
