...
class TrainerIncremental(nn.Module):
    ...
    def training_step(self, queries, keys, memories, labels):
        ...
        _, projectors = self.model_new_q(queries)
        projectors = nn.functional.normalize(projectors, dim=1)
        _, projectors_memories = self.model_new_q(memories)
        ...
        with torch.no_grad():
            ...
            _, projectors_memories_frozen = self.model_frozen(memories)
            projectors_memories_frozen = nn.functional.normalize(projectors_memories_frozen, dim=1)
        
        # adjust K
        max_non_neg = 0
        for label in labels:
            if len(self.dataloader.dataset.queries[label].non_negatives) > max_non_neg:
                max_non_neg = len(self.dataloader.dataset.queries[label].non_negatives)
        real_K = self.K if len(self.queue_pcd_index) - max_non_neg > self.K else len(self.queue_pcd_index) - max_non_neg
        ...
        for index in range(len(labels[:2*anchor_size])):
            negative_index = random.sample(list(self.queue_pcd_index.difference(set(self.dataloader.dataset.queries[labels[index]].non_negatives))), real_K)
