...
def evaluate(model, env_idx):
    ...
    for env in configs.eval.environments.keys():
        if configs.eval.environments[env]['stage_introduced'] <= env_idx or env_idx == -1: # Only eval on visited environments
            ...
            scene_id=configs.eval.environments[env]['stage_introduced']
            env_recall_1 = []
            for d, q in zip(database_files, query_files):
                if q != None:
                    env_recall_1.append(eval_multisession(model, d, q, scene_id)['Recall@1'])
                else:
                    ...
                    env_recall_1.append(eval_singlesession(model, d, scene_id, world_thresh, false_pos_thresh, time_thresh)['Recall@1'])
...
if __name__ == '__main__':
    ...
    model = model_factory(ckpt = torch.load(args.ckpt),
                          use_prompt=configs.model.use_prompt,
                          use_scene_id=configs.model.use_scene_id)
