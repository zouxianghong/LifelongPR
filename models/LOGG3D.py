...
from misc.utils import make_sparse_tensor
...
class LOGG3D(nn.Module):
    ...
    def forward(self, batch):
        assert 'coords' in batch.keys(), 'Error: Key "Cloud" not in batch keys.  Set model.mink_quantization_size to "None" to avoid!'
        x = batch['coords']
