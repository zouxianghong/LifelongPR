...
class PatchAugNet(nn.Module):
    ...
    def forward(self, batch):
        # task1: global descriptor
        x = batch['cloud']  # B x N x 3
        embedding = batch['features']  # B x N x 8
        # fp_features: [B x 256 x 128, B x 256 x 1024, B x 256 x 4096]
        res = self.backbone(x, embedding)
...
class PointNet2(nn.Module):
    ...
    def forward(self, pointcloud: torch.cuda.FloatTensor, feature: torch.cuda.FloatTensor):
        r"""
        ...
        for i in range(-1, -(len(self.FP_modules) + 1), -1):
            if i == -len(self.FP_modules) and feature is not None:
                l_features_i_1 = pointcloud + feature
                l_features_i_1 = l_features_i_1.transpose(1, 2).contiguous()  # B x 3 x N
                l_features[i - 1] = self.FP_modules[i](l_xyz[i - 1], l_xyz[i], l_features_i_1, l_features[i])
            else:
