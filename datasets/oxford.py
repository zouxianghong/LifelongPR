...
class OxfordDataset(Dataset):
    ...
    def __getitem__(self, ndx):
        ...
        # print("get item from the dataset")
        return query_pc, ndx
    ...
    def add_memory(self, memory):
        if memory != None:
            memory_queries_dict = memory.get_tuples(self.__len__())
            self.queries.update(memory_queries_dict)
            print(f'Updated: Added {len(memory_queries_dict)} Queries to form {len(self.queries)} Queries in Dataset')
...
class TrainingTuple:
    ...
    def __init__(self, id: int, timestamp: int, rel_scan_filepath: str, positives: np.ndarray,
        ...
        # positives: sorted ndarray of positive elemendef load_pc(self, filename):
        # Load point cloud, does not apply any transform
        # Returns Nx3 matrix
